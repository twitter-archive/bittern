...
/*! \todo call either x64 or x86 version depending on machine word size */
uint128_t murmurhash3_128(const void *buf, size_t sz)
{
	uint128_t ret;

	MurmurHash3_x64_128(buf, sz, 0U, &ret);
	return ret;
}
#ifdef __KERNEL__
...
/*! \todo call either x64 or x86 version depending on machine word size */
uint128_t murmurhash3_128_seed(uint32_t seed, const void *buf, size_t sz)
{
	uint128_t ret;

	MurmurHash3_x64_128(buf, sz, seed, &ret);
	return ret;
}
#ifdef __KERNEL__
EXPORT_SYMBOL(murmurhash3_128_seed);
#endif
